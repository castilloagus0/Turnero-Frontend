...
import { getTurnos } from '../../service/turno.service'
import { ADMIN_LIST_PAGE_SIZE } from './dashboardAdminListaActividadUi'

export function useAdminTurnos(pageSize: number = ADMIN_LIST_PAGE_SIZE) {
  ...
  const cargarTurnos = useCallback(async () => {
    ...
    try {
      const { turnos: lista, meta } = await getTurnos(pagina, pageSize)
      setTurnos(lista)
